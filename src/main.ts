...
import PluginExt from "./plugin-extend"
import { MyPluginSettings, SampleSettingTab, DEFAULT_SETTINGS } from "./settings"
import { SampleModal } from "./modals"
import { createWorker } from "./workers/helper"

import "./globals.css"
import { MY_VIEW, MyView } from './view';
...
export default class MyPlugin extends PluginExt<MyPluginSettings> {
  ...
  async registerSomething() {
    ...
    // Register a worker thread to do heavy computations
    this.register((() => {
  ...
  async addComponents() {
    ...
    const ribbonIconEl = this.addRibbonIcon('bot', 'Sample Plugin', (evt: MouseEvent) => {
      this.activateView(MY_VIEW, "right")
